...
{
# check for semaphore
if [ -f "${BARCHART_HOME}/util/updates/update-2.9-tasks.lock" ]; then
	echo "previously run"
else
	touch "${BARCHART_HOME}/util/updates/update-2.9-tasks.lock"
...
}  >> ${BARCHART_HOME}/logs/${YEAR}-${MONTH}-${DAY}-update-2.9-tasks.out 2>> ${BARCHART_HOME}/logs/${YEAR}-${MONTH}-${DAY}-update-2.9-tasks.err
# ---------------------------------------------------
