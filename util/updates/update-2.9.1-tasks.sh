...
{
# check for semaphore
if [ -f "${BARCHART_HOME}/util/updates/update-2.9.1-tasks.lock" ]; then
	echo "previously run"
else
...
}  >> ${BARCHART_HOME}/logs/${YEAR}-${MONTH}-${DAY}-update-2.9.1-tasks.out 2>> ${BARCHART_HOME}/logs/${YEAR}-${MONTH}-${DAY}-update-2.9.1-tasks.err
# ---------------------------------------------------
